[English](README.md) | [繁體中文](README.zh-TW.md)

# YouTube Study Companion v2.0

專業的 YouTube 播放控制與學習工具。將您的 YouTube 體驗轉變為強大的學習和筆記工作站。

📖 **[閱讀使用手冊](docs/USER_MANUAL.zh-TW.md)** 以獲得完整指南。

## 📥 安裝說明

本專案是一個瀏覽器擴充功能 (Browser Extension)，目前支援 Chrome 與 Edge 瀏覽器。請依下列步驟以開發者模式安裝：

1. **取得程式碼**
   
   下載此專案的 ZIP 檔並解壓縮，或是使用 Git Clone：
   ```bash
   git clone https://github.com/victor-hsu0618/youtube-sidebar-control.git
   ```

### 📦 安裝步驟 (建議)
1. **下載儲存庫**：點擊綠色的 "Code" 按鈕並選擇 **Download ZIP**。
2. **下載預先打包的擴充功能**：或者，您可以直接下載位於儲存庫根目錄的 [YouTubeStudioSideBar.zip](YouTubeStudioSideBar.zip)。
3. **解壓縮 ZIP**：在您的電腦上解壓縮該檔案。
4. **載入至 Chrome**：
   - 開啟 Chrome 並進入 `chrome://extensions/`
   - 開啟右上角的 **開發者模式** (Developer mode)。
   - 點擊 **載入未打包項目** (Load unpacked) 並選擇解壓縮後的資料夾。

2. **開啟擴充功能管理頁面**
   
   - **Chrome**: 在網址列輸入 `chrome://extensions`
   - **Edge**: 在網址列輸入 `edge://extensions`

3. **啟用開發人員模式**
   
   在頁面角落（通常在右上角）找到並開啟「開發人員模式 (Developer mode)」開關。

4. **載入未封裝擴充功能**
   
   - 點擊「載入未封裝項目 (Load unpacked)」。
   - 選擇本專案的資料夾（確保該資料夾內含有 `manifest.json` 檔案）。

5. **開始使用**
   
   安裝完成後，開啟任一 YouTube 影片頁面，點擊瀏覽器工具列中的 Extension 圖示，或開啟側邊欄即可使用。

## 🌟 主要功能

### 1. 🎛️ 進階播放控制
- **雙面板工作區**：專用的 **Markers (標記)** 與 **Controls (控制)** 子分頁，最大化垂直空間
- **摺疊式控制面板**：乾淨的介面，速度與循環區塊皆可摺疊收納
- **整合式 A-B 循環**：直接在進度條上顯示 **A/B 視覺標記**，可精確設定循環起點與終點
- **精確速度控制**：摺疊式滑桿調整 (0.25x 至 3.0x)
- **細部導航**：10 秒倒退/快進，或直接跳轉至特定時間點
- **托拽式進度條**：視覺化進度條，快速定位播放位置

### 2. 🔖 智慧標記
- **時間戳記筆記**：在影片任何時刻新增標記
- **立即跳轉**：點擊標記即可精確跳轉至該時間點
- **快速循環**：一鍵將標記時間套用至 A-B 循環起點
- **標記追隨 (Follow)**：可選的自動捲動功能，確保當前標記始終可見，設定具備**永久記憶**
- **分組管理**：將標記整理至 "Default", "Study" 或自定義分組
中

### 3. 💾 影片庫與我的最愛系統
- **影片收藏**：為單一 YouTube 影片儲存多個「工作階段」
    - *範例：一個收藏用於「單字」，另一個用於「文法分析」*
- **我的預設 (★)**：將您最愛的收藏標記為預設
- **自動偵測 🪄**：一鍵智慧載入當前影片的「預設」或「最近編輯」收藏
- **我的最愛分頁**：專門檢視所有已加星號/預設的收藏

### 4. 🪟 浮動視窗（彈出視窗）
- **多螢幕支援**：將側邊欄彈出為浮動視窗，可放置在第二個螢幕上
- **即時同步**：彈出視窗中的操作會立即與側邊欄同步（反之亦然）
- **自動關閉**：關閉側邊欄時會自動清理彈出視窗
- **切換模式**：彈出按鈕可輕鬆開啟/關閉外部視窗

### 5. 🧠 智慧連線
- **連線狀態指示燈**：即時回饋偵測狀態（綠燈 = 已連線，灰燈 = 待機）
- **待機模式**：當離開 YouTube 頁面時自動偵測
- **首頁引導**：在 YouTube 首頁時顯示實用的操作提示
- **返回喚醒**：當回到 YouTube 影片頁面時立即重新連線並喚醒控制
- **意圖導向載入**：當您在影片庫中點擊特定收藏時，extension 會確保載入該特定版本，即使您導航離開也一樣
- **鍵盤強大使用者支援**：完整的鍵盤快捷鍵，支援播放、循環、標記導航（詳見 UI 中的快捷鍵指南）

## 🛠️ 使用方式

1. **開啟**：在 YouTube 頁面點擊 extension 圖示
2. **控制**：使用頂部播放器區域控制速度和循環
3. **儲存**：點擊**愛心/儲存圖示**將當前影片加入您的影片庫
4. **彈出**：點擊標題列的 ↗️ 箭頭以分離視窗
5. **管理**：使用「影片庫」分頁檢視歷史記錄，或使用「我的最愛」分頁檢視已加星號的工作階段

## 📝 注意事項

本 extension 支援完整的鍵盤快捷鍵操作。點擊標題列的 **鍵盤圖示** 即可查看完整的 **快捷鍵指南**。大部分快捷鍵與 YouTube 原生邏輯一致 (Space/K, J/L, 0-9)，部分為本工具專屬 ([, ], {, }, 方向鍵上下)。
