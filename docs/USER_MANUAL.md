...
*   **Player (播放器)**：主要的操作區域，包含播放控制、速度調整、循環與標籤。
...
*   **Video Status**: 顯示目前偵測到的影片標題。
    ...
    *   **Save/Heart (❤️)**: 點擊愛心圖示，將目前影片的設定（標籤、循環點）儲存到影片庫。
*   **Transport Controls**:
    *   **Restart**: 回到影片開頭。
...
## 4. 智慧標籤 (Smart Bookmarks)

標籤功能讓您在影片的時間軸上做筆記，並隨時跳轉回去。

![標籤區域截圖](images/bookmarks.png)
*(建議截圖：Bookmarks 區域，包含幾個已建立的標籤)*

*   **新增標籤**：點擊 `+ Add Tag to Current Group`，即可在當下時間點建立標籤。
*   **分組管理 (Groups)**：
    *   透過下拉選單選擇分組（例如：`Formal`, `Study`）。
    *   不同性質的筆記可以分開存放。
*   **點擊跳轉**：點擊列表中的標籤，影片會立刻跳轉到該時間點。
*   **匯入/匯出**: 使用上方的下載/上傳圖示，備份您的標籤資料 (JSON 格式)。
